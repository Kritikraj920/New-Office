...
interface CODCurvePoint {
  tenor: string; // e.g. '1M', '3M'
  rate: number;
  days: number;
}

export async function runCODCurveValuation(batchId: string) {
  // 1. Get all CERTIFICATE OF DEPOSIT from FIMMDA
  const tBills = await prisma.FIMMDA.findMany({
    where: {
      uploadBatchId: batchId,
      category: { equals: 'CERTIFICATE OF DEPOSIT', mode: 'insensitive' },
    },
  });
  if(tBills.length==0){
  ...
  // 2. Get all points from CERTIFICATE OF DEPOSIT CURVE
  const curvePoints = await prisma.CDCurve.findMany({
    where: { uploadBatchId: batchId },
  });
  if(curvePoints.length==0){
    console.log("No COD data Found");
    return [];
  }
  const curve: CODCurvePoint[] = curvePoints
    .map(point => {
    ...
    .filter(Boolean) as CODCurvePoint[];
...
function interpolateYield(curve: CODCurvePoint[], targetDays: number): number {
