...
import { CD_CURVE_COLUMN_MAPPING, ProcessingResult } from '../valuationTypes';
import { BaseFileProcessor } from './BaseFileProcessor';

export class CODProcessor extends BaseFileProcessor {
  ...
  async processFile(file: Express.Multer.File, batchId: string): Promise<ProcessingResult> {
    try {
      ...
      await this.prisma.cDCurve.deleteMany({});

      let processed = 0, errors = 0;
      for (const record of allRecords) {
        try {
          const processedRecord = this.processRecord(record, batchId);
          await this.prisma.cDCurve.create({ data: processedRecord });
          processed++;
        } catch (err) {
      ...
      console.error('CD_Curve processing failed:', error);
      throw error;
    }
  }
  ...
  private mapRowToRecord(headers: string[], row: any[]): any {
    ...
    headers.forEach((header, idx) => {
      const dbCol = CD_CURVE_COLUMN_MAPPING[header];
