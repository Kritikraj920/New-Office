...
import { ProcessingResult, SLV_COLUMN_MAPPING } from '../valuationTypes';
import { BaseFileProcessor } from './BaseFileProcessor';

export class SLVProcessor extends BaseFileProcessor {
 ...
 async processFile(file: Express.Multer.File, batchId: string): Promise<ProcessingResult> {
  try {
    ...
    await this.prisma.sLV.deleteMany({});
    ...
    for (const record of allRecords) {
      try {
        const processedRecord = this.processRecord(record, batchId);
          ...
          await this.prisma.sLV.create({ data: processedRecord });
          processedRecords++;
        
      } catch (error) {
        console.error('Error processing SLV record:', error);
        errorRecords++;
      }
    }
    ...
    console.error('Error processing SLv file:', error);
    throw error;
  }
}
  ...
  private async parseFile(file: Express.Multer.File): Promise<any[]> {
    ...
    if (!sheetName) {
      throw new Error('Details sheet not found in SLV file');
    }
    ...
    const headerRowIndex = this.findHeaderRow(jsonData);
    if (headerRowIndex === -1) {
      throw new Error('Could not find header row in SLV file');
    }
  ...
  for (let i = 0; i < Math.min(data.length, 20); i++) {
    ...
    const hasPrice = lowerRow.some(h => h.includes('final price'));
    const hasMaturity = lowerRow.some(h => h.includes('maturity date'));
    const hasYTM = lowerRow.some(h => h.includes('final yield')); 
  ...
  private mapRowToRecord(headers: string[], row: any[]): any {
    ...
    headers.forEach((header, index) => {
      const dbColumn = SLV_COLUMN_MAPPING[header];
    ...
    if (!record.isin || !record.finalyield || !record.finalprice) {
      return null;
    }
  ...
  private processRecord(record: any, batchId: string): any {
    ...
    record={
      ...
      maturityDate: this.parseDate(record.maturutydate),
      rating:record.rating,
      segmentofissuer:record.segmentofissuer,
      modelyield:parseFloat(record.modelyield),
      modelprice:parseFloat(record.modelprice),
      yield15days:parseFloat(record.yield15days),
      price15days:parseFloat(record.price15days),
      finalyield:parseFloat(record.finalyield),
      finalprice:parseFloat(record.finalprice),
